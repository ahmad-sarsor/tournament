...
// هل يملك المستخدم صلاحية تعديل هذا التورنير؟ (منصّة، أو مالكه، أو مدير معيّن فيه)
function canEditTournament(tr) {
  if (!tr) return false;
  if (isPlatformAdminUser) return true;
  const e = myEmail();
  if (!e) return false;
  // admin_emails مخزّنة بحروف صغيرة (كما في القاعدة)؛ owner_email بنفس صيغة بريد المالك
  return tr.owner_email === e || (Array.isArray(tr.admin_emails) && tr.admin_emails.includes(e.toLowerCase()));
}
...
async function autoFinishStale() {
  if (!isMemberUser || autoFinishing) return;   // من له صلاحية تعديل فقط، ولا فحصين متزامنين
  autoFinishing = true;
  try {
    const now = Date.now();
    for (const m of await api.fetchLiveMatches()) {
      if (m.status === "live" && m.live_started_at && now - m.live_started_at >= AUTO_FINISH_MS) {
        // كل مباراة على حدة: قد تُرفض مباريات تورنيرات لا نملكها (القواعد)، فلا نوقف البقية
        try {
        ...
        } catch (_) { /* لا صلاحية على هذا التورنير — نتجاهل */ }
      }
    }
...
async function renderHome() {
  ...
  for (const tr of tournaments) {
    ...
    const canDelete = isPlatformAdminUser || tr.owner_email === myEmail();
...
async function renderUsersAdmin() {
  ...
  for (const u of users) {
    ...
    const padmin = owner || adminSet.has(u.email);        // مدير منصّة
    const member = padmin || memberSet.has(u.email);      // عضو معتمَد (المدير عضو تلقائياً)
    ...
    const actions = [];
    if (!owner) {
      // اعتماد كعضو (يختفي إن كان مدير منصّة لأنّه عضو تلقائياً)
      if (!padmin) actions.push(memberSet.has(u.email)
      ...
      // مدير منصّة (كل الصلاحيات)
      actions.push(adminSet.has(u.email)
...
async function renderTournamentAdmin(id, tab) {
  ...
  if (!canEditTournament(tournament)) return mount(app,
...
function renderDetailsTab(host, state) {
  ...
  const canManageAdmins = isPlatformAdminUser || tr.owner_email === myEmail();
  const canDelete = isPlatformAdminUser || tr.owner_email === myEmail();
  mount(host,
    ...
    canManageAdmins ? tournamentAdminsCard(tr) : null,
  );
}

// إدارة مدراء التورنير (يعيّنهم مالك التورنير بالبريد) — القائمة مخزّنة في admin_emails
function tournamentAdminsCard(tr) {
  const emails = Array.isArray(tr.admin_emails) ? tr.admin_emails.slice() : [];
  const input = el("input.input", { type: "email", placeholder: t.adminEmailPlaceholder, style: "flex:1" });
  const listHost = el("div");

  const renderList = () => mount(listHost, ...(emails.length
    ? emails.map((em) => el("div.admin-list-item", {}, [
        el("div.grow", { style: "font-weight:600", text: em }),
        el("button.btn.btn-sm.btn-danger", { text: t.remove, onclick: () => save(emails.filter((x) => x !== em)) }),
      ]))
    : [el("p.page-sub", { style: "padding:4px 2px", text: t.noTournamentAdmins })]));

  async function save(next) {
    try {
      await api.updateTournament(tr.id, { admin_emails: next });
      emails.length = 0; emails.push(...next); tr.admin_emails = next.slice();
  ...
  return el("div.card.card-pad", { style: "margin-top:14px" }, [
    el("div", { style: "font-weight:800;margin-bottom:4px", text: "👥 " + t.tournamentAdmins }),
    el("p.page-sub", { style: "margin:0 0 12px", text: t.tournamentAdminsHint }),
    listHost,
