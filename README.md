...
אתר לניהול טורנירים (בערבית, RTL) — לוח משחקים + טבלאות דירוג שמתעדכנות אוטומטית, פאנל ניהול להזנת תוצאות, ותמיכה בכמה טורנירים. כולל **מסבקת תחזיות** «مسابقة التوقّعات»: כל משתתף מקבל username ייחודי. חשבון עם מייל מאומת פעיל מיד; חשבון בלי מייל נשאר בהמתנה עד אישור הבעלים. המשתתפים מנחשים תוצאות משחקים (ננעל אוטומטית עם תחילת המשחק או מועדו), ומתחרים על טבלת דירוג ופרסים.
...
3. בתפריט הצד: **Build → Authentication** → **Get started** → בכרטיסייה **Sign-in method**:
   ...
   - אין צורך להפעיל **Anonymous** למסבקת התחזיות. חשבון בלי מייל נוצר עם username וסיסמה, אבל צריך אישור בעלים לפני השתתפות/יצירת טורניר.
