...
אתר לניהול טורנירים (בערבית, RTL) — לוח משחקים + טבלאות דירוג שמתעדכנות אוטומטית, פאנל ניהול להזנת תוצאות, ותמיכה בכמה טורנירים. כולל **מסבקת תחזיות** «مسابقة التوقّعات»: הקהל נרשם (שם משולש + טלפון/מייל + גיל), מנחש תוצאות משחקים (ננעל אוטומטית עם תחילת המשחק או מועדו), ומתחרה על טבלת דירוג ופרסים.
...
3. בתפריט הצד: **Build → Authentication** → **Get started** → בכרטיסייה **Sign-in method**:
   ...
   - הפעל גם את **Anonymous** → Save. *(דרוש ל‑מסבקת התחזיות «مسابقة التوقّعات» — מאפשר לקהל להשתתף בלי חשבון מלא.)*
   - הפעל גם את **Google** (בחר Project support email) → Save. *(מאפשר את כפתור «המתابعة بحساب Google».)*
